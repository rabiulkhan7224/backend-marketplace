...
const globalRateLimiter = createProgressiveRateLimiter({
  ...
  initialBlockMs: 5 * 60 * 1000, // 5 minutes initial block
...
const getCorsOrigin = async (): Promise<string[]> => {
  try {
    ...
    if (value === undefined) {
      ...
      // value = cors?.origin || [];
      value = ['http://localhost:5173', 'http://localhost:3000','https://marketplace-pi-inky.vercel.app'];
    ...
    return ['http://localhost:5173', 'http://localhost:3000',"https://marketplace-pi-inky.vercel.app"]; // Fallback origins
  }
};
...
const getFallbackOrigins = (): string[] => [
  'http://localhost:5173',
  'http://localhost:3000',
  'https://marketplace-pi-inky.vercel.app'
];
